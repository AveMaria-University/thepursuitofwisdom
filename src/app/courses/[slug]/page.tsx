...
import { getCourseBySlug, getCoursePaths } from '@/data/courses';
import { notFound } from 'next/navigation';
import DropboxVideo from '@/components/DropboxVideo';
import LessonsList from './LessonsList';

interface CoursePageProps { params: { slug: string } }

// NOTE: Moved metadata generation to separate server file (metadata.ts) to allow this page to be a client component.
export function generateStaticParams() { return getCoursePaths(); }
...
export default function CourseLandingPage({ params }: CoursePageProps) {
	...
	// Course-specific background heights
	const getIntroBackgroundHeight = () => {
		if (course.slug === 'life-and-teachings-of-st-augustine') return 'h-[1050px]';
		if (course.slug === 'athletics-and-the-virtues') return 'h-[950px]';
		return 'h-[1000px]'; // default
	};

	// Course-specific card styling
	const getCardStyling = () => {
		if (course.slug === 'athletics-and-the-virtues') {
			return 'bg-white rounded-xl shadow-lg p-8 md:p-16 mb-24 relative overflow-hidden min-h-[600px]';
		}
		// Augustine and others use original styling
		return 'bg-white rounded-xl shadow-lg p-6 md:p-12 mb-24 relative overflow-hidden';
	};
		...
		<div className="min-h-screen bg-white">
			...
			<main className="relative">
					...
					<div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-0 relative z-10">
						...
						<div className={getCardStyling()}>
							...
							{(() => {
								...
								return (
									<div className={`relative z-10 max-w-5xl mx-auto space-y-6 text-center ${course.trailerUrl ? 'mt-10' : ''}`}>
				...
				<section 
					id="program" 
					className="relative"
					style={course.exploreImage ? {
						backgroundImage: `url(${course.exploreImage})`,
						backgroundSize: 'cover',
						backgroundPosition: 'center',
						backgroundRepeat: 'no-repeat'
					} : {}}
				...
				{/* Presenter Section - No gap for courses with explore backgrounds */}
				{(course.presenterBio || course.presenterImage) && (
					<section className={`relative left-1/2 right-1/2 -mx-[50vw] w-screen bg-pursuit-gold pb-0 ${course.exploreImage ? '' : 'mt-32'}`}>
